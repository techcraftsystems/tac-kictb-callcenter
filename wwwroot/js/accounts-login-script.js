jq(function() {
    ...
    //Press Enter
    jq('input').keyup(function(e){
        if(e.keyCode == 13){
            jq('form a.btn-login').click();
        }
    });

    //Validate Form before Submit
    jq('form a.btn-login').click(function() {
        ...
        if (un.hasClass('error') || pw.hasClass('error')) {
            M.toast({html:'<span>Ensure username/password is properly filled</span><a class="btn-flat yellow-text" href="#!">Try Again<a>'});
            return;
        }
